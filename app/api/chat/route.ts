...
export async function POST(req: Request) {
  const { messages, pageContext, search } = await req.json();
  ...
  const result = streamText({
    ...
    system: `${
      search
        ? `TASK: your task is help the user by making sure to search the web based on the user prompt also make sure to see the page data that the user is currently in data:${formattedPageContext}\n` +
          '\n\nRULES:\n' +
          '-always make sure to read the messages history to stay up to dated with the conversions\n' +
          '-make sure to obey only requested related to search products improve sales or any related topic in e commerce else answer with "I am not able to help you with that."'
        : `${
